...
Enum.each verbs, fn x ->
  Enum.each nouns, fn y ->
    patched_program = Map.replace(program, 1, x)
    patched_program = Map.replace(patched_program, 2, y)
    result = IntCodeCPU.execute(patched_program,0)
    if result == 19690720 do
      IO.puts("Ouput: 100*#{x}+#{y} = #{100*x+y}")
    end
  end
end
