...
function play(moves, boards){
  let winning_board_summaries = [];

  moves.forEach((move, move_ix) => {
    score_boards.forEach((board, board_ix) => {
      if(winning_board_summaries.find(summary => summary[0] == board_ix)){ return }

      new_board = board.map(val => val == move ? 'x' : val)
      boards[board_ix] = new_board
      if(is_winner(new_board)){
        winning_board_summaries.push([board_ix, move_ix, new_board, move])
      }
    });
  });
  return winning_board_summaries;
}
...
let winning_board_summaries = play(moves, score_boards)
let last_board = winning_board_summaries.pop();
console.log(score(last_board));