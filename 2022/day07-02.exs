...
used_space = Map.get(state.directory_sizes, "/")
free_space = disk_size - used_space
required = update_size - free_space

IO.inspect("Used #{used_space} free #{free_space} required #{required}")

Enum.map(state.directory_sizes, fn {_, v} -> v end)
  |> Enum.sort()
  |> Enum.find(fn v -> v > required end)
  |> IO.inspect()
